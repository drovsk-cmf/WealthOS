-- Migration 066: Implement real cron_process_account_deletions
-- Replaces stub (LIMIT 0) with full LGPD-compliant account deletion
-- Processes requests after 7-day grace period
-- Deletes from all 25 user tables + storage + auth.users
...
BEGIN
  -- Find users who requested deletion more than 7 days ago
  FOR v_user IN
    SELECT id
    FROM users_profile
  ...
  LOOP
    BEGIN
      ...
      -- Workflows (workflow_tasks CASCADE from workflows)
      DELETE FROM workflows WHERE user_id = v_user.id;

      -- Transactions (no more FK to journal_entries blocking)
...
GRANT EXECUTE ON FUNCTION cron_process_account_deletions() TO service_role;

COMMENT ON FUNCTION cron_process_account_deletions() IS
  'LGPD: processes account deletion requests after 7-day grace period. '
  'Deletes all user data from 25 tables, storage objects, and auth.users. '
  'Runs daily at 06:30 UTC via pg_cron. Each user deletion is isolated in its own subtransaction.';
