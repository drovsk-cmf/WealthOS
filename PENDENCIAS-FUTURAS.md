...
| E30 | **Estrutura de navegação (5 tabs + sininho)** — [Início] [Movimentações] [Patrimônio] [Orçamento] [Mais] + sininho topo direito. Impostos no "Mais" como 1º item. Ref: docs/NAVIGATION-SPEC.md | Baixo | Crítico (arquitetura) | ✅ (decisão) / ⬜ (implementação) |
...
| E40 | **Múltiplos métodos de orçamento** — oferecer ao menos 2 métodos: category budgeting (padrão atual) + flex budgeting (Monarch) ou zero-based (YNAB). WalletHub oferece 5 métodos. Monarch oferece 2. **Reclassificado para implementação imediata por decisão do Claudio.** GAP COMPETITIVO. | Médio | Médio (orçamento) | ⬜ |
| E50 | **Motor tributário PF robusto** — tabela `tax_parameters` versionada por ano-calendário com todos os parâmetros fiscais (IRPF, INSS, deduções, investimentos). Cálculos: IRPF mensal/anual, INSS, ganho de capital, projeção restituição, DARF. Lei 15.270/2025 (isenção R$ 5k, IRPFM alta renda). Validação cruzada com simulador RFB. Disclaimer obrigatório. **Implementação imediata por decisão do Claudio.** Ref: docs/TAX-ENGINE-SPEC.md | Alto | Alto (fiscal) | ⬜ |
| E51 | **Calendário fiscal completo** — IPVA (jan-mar por UF), IPTU (variável por município), IRPF (mar-mai), DARFs mensais (ganho capital), carnê-leão. Alertas da Onie 7 dias antes. Alimentado por dados do perfil (UF, município, veículos, investimentos). | Baixo | Alto (fiscal) | ⬜ |
...
| E45 | **Motor CLT (bruto → líquido automático)** — cálculo automático de INSS, IRRF, VT, VR, FGTS a partir do salário bruto. Facilita onboarding de usuários CLT. **Reclassificado de H2→H1 por decisão do Claudio.** Ref: adendo v1.4 + docs/TAX-ENGINE-SPEC.md | Médio | Médio (onboarding) | ⬜ |
...
| TEC-12 | **Chunking para import batch de alto volume** — o batch insert via RPC `import_transactions_batch` processa todas as transações em uma única chamada. Com 3.381 linhas (teste de estresse sessão 36) o botão não deu feedback e potencialmente excede timeout. Implementar: chunking de 500 linhas por batch + progress bar + retry por chunk. Ref: HANDOVER §36.7 item 3. | Média | Primeiro teste com volume > 1.000 transações | ⬜ |
| TEC-13 | **Regenerar `database.ts`** — tabela `bank_institutions` e novos campos em `accounts` (`bank_institution_id`, `branch_number`, `account_number`, `account_digit`) não estão nos types gerados. Hook `use-bank-institutions.ts` usa cast `as any` como workaround. Executar `supabase gen types` (requer `SUPABASE_ACCESS_TOKEN` env var). Ref: HANDOVER §36.7 item 4. | Baixa | Próxima sessão de desenvolvimento | ⬜ |
