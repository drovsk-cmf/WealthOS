...
| B19 | Calculadoras sem empty state: páginas /calculators/* não mostram mensagem explicativa quando dados são insuficientes. Usuário vê formulário sem contexto. Fix: adicionar empty state com orientação ("preencha X para calcular Y"). Auditoria 7.2 | 44 |
| B20 | color-contrast SERIOUS (WCAG AA) em ~8 páginas. `text-muted-foreground` gera contraste 3.57-4.38:1 (mínimo 4.5:1) em /dashboard, /transactions, /cash-flow, /bills, /assets, /tax, /diagnostics. Não falha o teste (SERIOUS, não CRITICAL) mas é violação real. Fix: ajustar `--muted-foreground` no tema ou usar opacidade maior. Auditoria 7.3 | 44 |
| B21 | 6 botões com touch target < 30px no dashboard mobile (390px). WCAG recomenda mínimo 44x44px. Fix: aumentar padding/tamanho dos botões afetados. Auditoria 7.7 | 44 |
