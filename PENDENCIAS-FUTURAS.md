...
| E66 | **Dedup learning loop (princípio #5 da DEDUP-ENGINE-SPEC)** — motor de deduplicação não aprende com decisões do usuário. Falta: registrar decisões, ajustar thresholds por padrão, exclusão por sinal oposto (princípio #6). Engine base funciona. Ref: auditoria A035 sessão 39. | 3-4h | Médio (qualidade de import) | ⬜ |
| E67 | **Motor de parcelamento (INSTALLMENT-SYSTEM-SPEC)** — schema existe (closing_day, due_day, credit_limit em accounts). Falta: aritmética do centavo, geração de N parcelas, distribuição por fatura, installment_of/installment_total em transactions. Ref: auditoria A036 sessão 39. | 4-6h | Alto (caso de uso dominante BR) | ⬜ |
| E68 | **Parsers bank-specific (IMPORT-ENGINE-SPEC §4)** — bank-detection identifica bancos, mas parsers individuais (BTG XLSX, XP CSV, Porto PDF, Itaú Samsung PDF, etc.) não existem. Parsers genéricos (csv/ofx/xlsx) cobrem parcialmente. Ref: auditoria A039 sessão 39. | 4-6h | Alto (usabilidade import) | ⬜ |
| E69 | **Password derivation para faturas criptografadas (IMPORT-ENGINE-SPEC §3.5)** — derivar senha de XLSX/PDF protegido a partir de CPF/CEP do usuário, conforme regras por banco. Ref: auditoria A039 sessão 39. | 2-3h | Médio (import BTG/Porto) | ⬜ |
| E70 | **Inbound email para faturas (IMPORT-ENGINE-SPEC §2)** — endereço dedicado por usuário, allowlist de remetentes, processamento automático de anexos. Requer serviço de inbound email (Resend/AWS SES). Ref: auditoria A039 sessão 39. | 6-8h | Alto (zero-touch import) | ⬜ |
| E71 | **Workflow para falhas de importação (IMPORT-ENGINE-SPEC §2.8)** — gerar workflow automático quando import falha (senha incorreta, formato desconhecido, remetente não autorizado). Ref: auditoria A039 sessão 39. | 2-3h | Médio (UX de erro) | ⬜ |
