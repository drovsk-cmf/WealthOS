...
|---|---|---|---|---|---|
| 1 | Auth & Sessão | D1.01 | ALTO | Rate limiter in-memory não compartilha estado entre instâncias serverless | Médio |
...
| 5 | Mobile | D4.03 | ALTO | Certificate pinning não implementado | Alto |
| 6 | Performance | D6.09 | ALTO | Funções SECURITY DEFINER sem SET search_path (migrations 001, 003) | Médio |
| 7 | Performance | D6.15 | ALTO | Trigger `recalculate_account_balance` faz full-table scan (O(n²) em imports) | Alto |
...
| 10 | UX | D7.03 | ALTO | Transações não podem ser editadas, apenas estornadas | Alto |
| 11 | Acessibilidade | D8.01 | ALTO | Dialogs customizados sem `role="dialog"`, `aria-modal` e focus trap | Alto |
...
| 18 | Auth & Sessão | D1.05 | MÉDIO | Verificação AAL2 (MFA) apenas client-side | Médio |
...
| 27 | Mobile | D4.02 | MÉDIO | `attemptBiometricUnlock()` bypass sem biometria real | Médio |
...
| 55 | Auth & Sessão | D1.06 | BAIXO | Login schema não aplica blocklist de senhas | Baixo |
...
| 60 | Mobile | D4.04 | BAIXO | Plugin biometric-auth ausente do package.json | Baixo |
...
| 66 | Código | D5.09 | BAIXO | TODO pendente para Fase 10 (biometria) | Baixo |
| 67 | Performance | D6.06 | BAIXO | useBudgetMonths sem DISTINCT no banco | Baixo |
| 68 | Performance | D6.07 | BAIXO | useMultiIndexHistory N RPCs paralelas vs query única | Baixo |
| 69 | Performance | D6.08 | BAIXO | useProgressiveDisclosure 7 queries COUNT simultâneas | Baixo |
| 70 | Performance | D6.12 | BAIXO | cron_fetch_economic_indices sem timeout HTTP | Baixo |
| 71 | Performance | D6.13 | BAIXO | useDeleteAsset exclusão manual de FK com CASCADE existente | Baixo |
| 72 | Performance | D6.14 | BAIXO | createTransaction 2 UPDATEs sequenciais após RPC | Baixo |
| 73 | Performance | D6.16 | BAIXO | Invalidação sequencial de query caches com await | Baixo |
